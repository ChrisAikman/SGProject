...
Although I implemented this project in the way I thought best, I am certain there are better ways to go about implementing much of the functionality. I look forward to hearing ways to improve and optimize this app.
...
Compile production build with: `npm run build`. It is set to build for my website [here](http://www.chrisaikman.com/sgproj). Change the homepage in the `package.json` config file to host it elsewhere.
...
After my family had to leave, I put in several hours to finalize most of the functionality, and styling.

### Tuesday, December 27, 2016
Minor fixes to polish everything for submission. Wrote a majority of this README.
...
### We Are Brady
Since one of the requirements was to assume that we are one of the users with at least two comments, and since Brady has the most comments, I assumed that we are the user Brady. This can be changed in the `index.js` script to any other user, or no user.
...
I assumed that in a real deployment of this app, it would be communicating back and forth with the server. It is possible to do realtime updating of comments by pulling in the database, reparsing it, and then updating the app state within React. In addition, when a user edits, deletes, or replies to a comment, a request should be made to the server to perform that operation within its database. These requests should be done within the `handleEdit()`, `handleReply()`, and `handleDelete()` functions within `app.js`. In addition, the server should check that the requests are valid.
...
There is potential security flaw with user ids and comment management, so it is assumed that the server will check the validity of a new comment, edit, or deletion based on the currently logged in user before making changes to the database. It is assumed that the end-user can change their user id within the app, and edit/delete as many posts as they want by spoofing an id, however, with proper server checking, the actions would not be mirrored on the server side. It is assumed that this is O.K., because any end-user with a tool such as firebug could do the same thing.
...
See the [We Are Brady](#we-are-brady) section under Assumptions.

### Display and implement Edit and Delete functionality for the appropriate comments.
This, in addition to Reply functionality, are all done within the App and Comment components, with the help of a few minor components such as CommentToolbutton and DeleteCommentWindow.
...
## Data Format
Upon initial viewing of the thread data, and reading on React data updating conventions, I decided it would be beneficial to parse the data into a new format. The new format involves two data structures: the `discussion` and `comments`. The `discussion` contains the comment key for the main discussion, and the title of the discussion. The discussion itself is converted into a comment. The `comments` structure is a dictionary of comments keyed by their individual comment keys. Each comment has its original data, except its comments array is converted to hold the keys of the subcomments. This new data structure makes it easy to search for any comment in the discussion by using its key. Helper functions are provided to quickly convert the incoming raw database into this new data format.

This new format was decided on from the beginning as React convention states that, "*When you want to aggregate data from multiple children or to have two child components communicate with each other, move the state upwards so that it lives in the parent component. The parent can then pass the state back down to the children via props, so that the child components are always in sync with each other and with the parent.*" So instead of splitting the data down to the comment components, it was maintained in the main App. This is not necessary and could be changed back to the original format with a few modifications to the code, but there would need to be additional functions to search the recursive database, and state updates would become complicated within the code. I could be wrong with these assumptions and would love to hear alternative methods of achieving the same goal.

## Design
For the design, I tried to keep everything as minimalistic as possible. I tried to make sure that every part of the app has a purpose and that structure is heavily implied. I focused on a gray color scheme with a blue primary highlight color. I included room for a profile picture since, if I recall correctly from Visualization class, a picture can be processed and recognized within the brain faster than making the connection between a user name and the user. I used enclosure to show which comments are subcomments. I used colors to emphasize certain features, such as coloring a comment that is going to be deleted red, and a new comment green. I used icons within the buttons for fast function recognition, and made the buttons stand out just enough to show that they provide functionality. I sized the comments based on the assumption that this app would likely have additional information or features on the side of the page. Finally, I utilized media queries to ensure that the app would respond to the sizes of different devices. Since I don't have access to a device testing suite, I tested the design on my computer, tablet and phone to ensure consistent use and easy access on varying sizes.
...
### Font-Awesome
I utilized font-awesome for the font symbols within the buttons. This was chosen because it is popular and well-supported.
...
### Scroll Into View
Since the default smooth scrolling code is not supported by all browsers, I needed to find a library that would give the functionality I needed. The `scroll-into-view` module gave me the options I needed to scroll specific nodes smoothly to the center to the window.
...
This is just one discussion thread. It could use a parent layer that displays all of the discussion threads within a topic, and a grandparent layer that displays all of the topics on the site. In addition, it would be beneficial to include more information, such as related discussions to allow for a more immersive experience for the user.

## What I've Learned
I learned so much during this project! The biggest thing is that I learned (or relearned) how to use all of the tools and modules that I utilized in this project. I relearned how to setup and build a React app, and learned some new conventions for it. I learned how to configure Webpack and use common modules within it, such as the React Updates module, and the Less CSS preprocessor module. I also finally learned how to do formal JavaScript testing!
